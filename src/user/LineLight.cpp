...
LineLight::LineLight(const std::vector<Vec3<float>*> &POS_ARRAY)
{
	...
	{
		model->m_meshes[i].material->constData.pbr.baseColor = { 1.0f,1.0f,1.0f };
		model->m_meshes[i].material->constData.lambert.emissive = { 0.0f,1.0f,0.0f };
	...
	{
		lightBlocks[i] = std::make_unique<LightBlock>(model);
	}
	lightBlockPosArray = POS_ARRAY;
...
void LineLight::Init()
{
	...
	{
		lightBlocks[i]->Init(*lightBlockPosArray[i]);
	}
}
